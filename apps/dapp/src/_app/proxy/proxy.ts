...
import type { TServerSessionData } from '@/entities/session';
import {
  SESSION_COOKIE_NAME,
...
export async function handleProxy(request: NextRequest): Promise<NextResponse> {
  ...
  const cspDirectives = [
    ...
    // Ark UI positions toast regions with runtime style attributes. Keep
    // <style> blocks nonce-restricted while allowing that narrower CSP class.
