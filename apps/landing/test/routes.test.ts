...
const readBuiltRoute = (path: string): Promise<string> =>
...
test('robots route publishes the configured local sitemap by default', async () => {
  const body = await readBuiltRoute('robots.txt');

  assert.equal(
    body,
    'User-agent: *\nAllow: /\n\nSitemap: http://localhost:46002/sitemap-index.xml\n',
  );
});
